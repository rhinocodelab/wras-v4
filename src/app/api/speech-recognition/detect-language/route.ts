...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Language detection request received:', { audioId });
    ...
    // Try different encodings and configurations
    const encodingConfigs = [
      { encoding: 'WEBM_OPUS' as const, sampleRate: 48000 },
      { encoding: 'LINEAR16' as const, sampleRate: 16000 },
      { encoding: 'LINEAR16' as const, sampleRate: 44100 }
    ];
        ...
        try {
          const speechRequest = {
            ...
            config: {
              ...
              sampleRateHertz: encodingConfig.sampleRate,
              languageCode: langConfig.primary,
              alternativeLanguageCodes: langConfig.alternatives,
          ...
          console.log(`Trying: ${encodingConfig.encoding}/${encodingConfig.sampleRate} with primary: ${langConfig.primary}`);
          const [response] = await speechClient.recognize(speechRequest);
          
          console.log(`Response for ${langConfig.primary}:`, {
          ...
          if (response.results && response.results.length > 0) {
            ...
            if (alternative) {
              ...
              console.log(`Primary: ${langConfig.primary}, Transcript: "${transcript}", Detected: ${detectedLang}, Confidence: ${confidence}`);
              
              // Score based on confidence and language match
              let score = confidence;
              
              // Boost score if detected language matches primary language
              if (detectedLang === langConfig.primary) {
                score += 0.2;
              }
              ...
              console.log(`No alternative found for ${langConfig.primary}`);
            }
          } else {
            console.log(`No results found for ${langConfig.primary}`);
          }
        } catch (error) {
          console.log(`Error with ${encodingConfig.encoding}/${encodingConfig.sampleRate} and ${langConfig.primary}:`, error);
        }
      }
    }
    ...
    console.log('Language detection completed:', {
      ...
      bestLanguage,
      bestScore,
      hasResults: bestResponse?.results?.length > 0
    });

    if (!bestResponse || !bestResponse.results || bestResponse.results.length === 0) {
      console.log('No speech detected in any language attempt');
      return NextResponse.json(
    ...
    // Get the best transcript for reference
    const bestResult = bestResponse.results[0];
    const bestAlternative = bestResult.alternatives?.[0];
    const bestTranscript = bestAlternative?.transcript || '';

    console.log('Returning language detection result:', {
      detectedLanguage: bestLanguage,
      confidence: bestScore,
      sampleTranscript: bestTranscript
    });
